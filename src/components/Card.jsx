...
<img alt="logo" src={item.logo} style={{marginLeft:'auto', marginRight:'0' , display:'table-row'}}/>
  <div style={{alignItems:'center' , display:'flex'}} ><img alt="arrow" src={item.up?'./images/icon-up.svg':'./images/icon-down.svg'}/><span className={item.up?'green':'red'}> {item.per}%</span> </div>
</div>
</div>	)})}
</div>)}
export default Card